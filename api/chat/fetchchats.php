...
if (isset($data) && isset($_COOKIE["token"])) {
    ...
    if ($check_user->num_rows === 0) {
        ...
        if ($check_is_following->num_rows > 0 && $check_is_being_followed->num_rows > 0) {
            $conn->query("UPDATE messageData SET status = 'read' WHERE (messageFROM = '$userUID' AND messageTO = '$sender') OR (messageFROM = '$sender' AND messageTO = '$userUID')");
            $messages = $conn->query("SELECT * FROM messageData WHERE (messageFROM = '$userUID' AND messageTO = '$sender') OR (messageFROM = '$sender' AND messageTO = '$userUID') ORDER BY sentOn DESC");
            if ($data['type'] !== "first") {
                $last_message = $data['last'];
                $messages = $conn->query("SELECT * FROM messageData WHERE (messageFROM = '$userUID' AND messageTO = '$sender') OR (messageFROM = '$sender' AND messageTO = '$userUID') AND messageID < '$last_message' ORDER BY sentOn DESC LIMIT 25");
            }
            $total = 0;
            $message_details = [];
