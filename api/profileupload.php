...
if ($rawData && isset($_COOKIE["token"])) {
    ...
    if (isset($_COOKIE["token"])) {
        ...
        if ($check_user->num_rows === 0) {
            ...
            $imageTmpName = $profileImage['contents'];
            $imageSize = strlen($imageTmpName);
            $imageError = 0;
            $imageType = $profileImage['headers']['content-type'];
            ...
            $new_image_name = "socioy-user-profile" . uniqid() . uniqid() . uniqid() . uniqid() . uniqid() . "." . $file_ext;
            if (file_put_contents("../cdn/" . $new_image_name, $imageTmpName)) {
                $insert_profile_image_query = $conn->query("UPDATE loginInfo SET profileImage = '$new_image_name' WHERE userUID = '$userUID'");
                echo json_encode((array("status" => "success", "message" => "profile upload success", "profile" => "$new_image_name")));
                die();
            } else {
